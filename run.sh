...
if [ "$APP_CHECK" != "" ]; then
	...
	docker stop $APP_CHECK
	docker rm $APP_CHECK
fi;

docker run -dt --volumes-from $1-storage -p 8888:8888 --name $1-application -h $1 -e VIRTUAL_HOST=$1 bradleyboy/docker-koken-nginx
echo "Koken container started."
